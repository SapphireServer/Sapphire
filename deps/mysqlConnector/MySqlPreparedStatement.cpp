...
namespace Mysql
{
  ...
  struct LongDataSender {
    ...
    LongDataSender( MYSQL_STMT* pStmt, unsigned int i ) : position( i ),
                                                          m_pStmt( pStmt )
    {
    }
    ...
    bool operator()( std::istream* my_blob ) const
    {
      ...
      do
      {
        ...
        {
          switch( mysql_stmt_errno( nativeStatement( m_pStmt ) ) )
          {
...
Mysql::PreparedStatement::PreparedStatement( NativeStatementHandle pStmt, std::shared_ptr< Mysql::Connection > pConn )
    : Statement( pConn )
{
  m_pStmt = pStmt;
  m_pConnection = pConn;
  m_paramCount = mysql_stmt_param_count( nativeStatement( m_pStmt ) );
  m_pParamBind.reset( new ParamBind( m_paramCount ) );
  m_pResultBind.reset( new ResultBind( nativeStatement( pStmt ) ) );
}

uint32_t Mysql::PreparedStatement::errNo()
{
  return mysql_stmt_errno( nativeStatement( m_pStmt ) );
}
...
bool Mysql::PreparedStatement::sendLongDataBeforeParamBind()
{
  ...
  for( unsigned int i = 0; i < m_paramCount; ++i )
  {
    ...
    {
      LongDataSender lv( nativeStatement( m_pStmt ), i );
...
void Mysql::PreparedStatement::doQuery()
{
  ...
  if( mysql_stmt_bind_param( nativeStatement( m_pStmt ), m_pParamBind->getBindObject() ) )
    throw std::runtime_error( "Couldn't bind : " + std::to_string( errNo() ) );

  if( !sendLongDataBeforeParamBind() || mysql_stmt_execute( nativeStatement( m_pStmt ) ) )
    throw std::runtime_error( "Couldn't execute : " + std::to_string( errNo() ) + ": " + std::string( mysql_stmt_error( nativeStatement( m_pStmt ) ) ) );
...
{
  if( m_pStmt )
    mysql_stmt_close( nativeStatement( m_pStmt ) );
  clearParameters();
}
