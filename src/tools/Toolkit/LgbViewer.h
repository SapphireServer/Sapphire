...
public:
  ...
  std::string getEntryTypeString( eAssetType type );

  std::string getEntryName( InstanceObjectEntry *entry );

  void onEntrySelected( InstanceObjectEntry *entry );
...
private:
  ...
  {
    ImGuiID id;
    std::shared_ptr< InstanceObjectEntry > entry;
    bool isOpen;
    std::string windowTitle;

    EntryViewWidget( std::shared_ptr< InstanceObjectEntry > e ) : entry( e ), isOpen( true )
    {
  ...
  void showSGEntryView( SharedGroupEntry *entry );

  void showEObjEntryView( EventObjectEntry *entry );
  ...
  void openEntryViewWidget( std::shared_ptr< InstanceObjectEntry > entry );
};
